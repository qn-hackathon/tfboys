...
class TestTextAnalyzer:
    @pytest.fixture
    def text_analyzer(self, mock_anthropic_client):
        """创建TextAnalyzer实例"""
        with patch('app.services.text_analyzer.Anthropic', return_value=mock_anthropic_client):
    ...
    async def test_analyze_novel_success(self, text_analyzer, sample_novel_text, sample_scenes_data):
        ...
        mock_response.content = [MagicMock(text=json.dumps({"scenes": sample_scenes_data}))]
        text_analyzer.client.messages.create = MagicMock(return_value=mock_response)
