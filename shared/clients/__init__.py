...
def get_redis_client():
    """
    获取 Redis 客户端实例（动态获取，解决模块导入时的值复制问题）

    Returns:
        RedisClient: Redis 客户端实例，如果未初始化则返回 None
    """
    import sys
    # 直接从 sys.modules 获取子模块
    rc_module = sys.modules.get('shared.clients.redis_client')
    if rc_module is None:
        # 如果模块未加载，先导入
        import importlib
        rc_module = importlib.import_module('shared.clients.redis_client')
    return rc_module.redis_client
